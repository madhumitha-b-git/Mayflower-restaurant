...
import { BrandHeader } from './hr/BrandHeader';
import { StatCards } from './hr/StatCards';
import { FilterBar, ActiveTab } from './hr/FilterBar';
import { StaffTable } from './hr/StaffTable';
import { StaffMobileCardList } from './hr/StaffMobileCardList';
import { LeaveRequestsView } from './hr/LeaveRequestsView';
import { ActivityLogView } from './hr/ActivityLogView';
import { OnboardModal } from './hr/OnboardModal';
...
import { Employee, LeaveRequest, ActivityLog, OutletName, DutyStatus } from './hr/types';
...
export const HRDashboard: React.FC<Props> = ({ user, onLogout, onSwitchRole }) => {
  ...
  const handleUpdateDutyStatus = (employeeId: string, newStatus: DutyStatus) => {
    ...
    const newLog: ActivityLog = {
      ...
      details: `Status set to ${newStatus} by ${user.name || 'HR Director'}`,
  ...
  return (
    <div className="min-h-screen bg-luxury-dots flex flex-col font-sans pb-12">
      {/* Main Content Area */}
      <main className="flex-1 w-full max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-6 space-y-4 sm:space-y-6">
        {/* Green Brand Banner Card with M · FD Logo, Role switcher, + ONBOARD, and EXIT */}
        <BrandHeader
          userName={user.name}
          userRole={user.role || 'HR Director'}
        ...
        {/* Navigation Tabs, Outlet Location Filter, and Search Bar */}
        <FilterBar
          activeTab={activeTab}
        ...
        {activeTab === 'directory' && (
          <div>
            {/* Responsive: Table on medium/large screens, Cards on mobile */}
